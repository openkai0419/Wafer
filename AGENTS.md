# Instructions for contributors especcialy codex
- When modifying Python file under `/source`, or if you find `/tests` is not optimal for `/source` , update the matching test in `/tests`.
- Check each test file for compatibility with the source and update or rewrite the test file for optimal testing.
- Keep and modify the `/tests` directory will be same structure as `source` for visibility.
