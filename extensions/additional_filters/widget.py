...
class _CalendarPopup(PopupBase):
    ...
    def show_at(self, global_pos: QtCore.QPoint, current_date: QtCore.QDate | None = None):
        today = QtCore.QDate.currentDate()
        self._calendar.setMaximumDate(today)

        palette = ThemeManager.instance().palette
        self._calendar.setStyleSheet(f"QCalendarWidget QAbstractItemView::item:selected{{ background-color: {palette.accent};  color: {palette.accent_text}; }}")

        self._calendar.setDateTextFormat(QtCore.QDate(), QtGui.QTextCharFormat())
        today_fmt = QtGui.QTextCharFormat()
        today_fmt.setFontWeight(QtGui.QFont.Bold)
        hl = self.palette().color(QtGui.QPalette.Highlight)
        today_fmt.setForeground(self.palette().color(QtGui.QPalette.HighlightedText))
...
class _DateInput(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        ThemeManager.instance().on_theme_changed.connect(lambda _: self._drop_btn.setIcon(themed_icon("chevron_down")))
    ...
    def _show_popup(self):
        if self._popup is None:
            ...
            self._popup.popup_closed.connect(lambda: self._drop_btn.setDown(False))
        current = self._date if self._state == "date" else QtCore.QDate.currentDate()
...
class DateRangeWidget(QtWidgets.QWidget):
    ...
    def _build_ui(self):
        ...
        self._preset_widgets = [self.preset_value, self.preset_unit, self.preset_from_label, self.preset_ref]
