...
def _recursive_partition(rects, cancelled, sorted_ar, ar_prefix, inv_prefix, start, count, x, y, w, h, spacing, split_point_fn, ratio_fn):
    ...
    ratio = ratio_fn(sorted_ar, ar_prefix, inv_prefix, start, count, left_count, split_x)

    if split_x:
        left_w = max(1, int(w * ratio) - spacing // 2)
        right_w = max(1, w - left_w - spacing)
        _recursive_partition(rects, cancelled, sorted_ar, ar_prefix, inv_prefix, start, left_count, x, y, left_w, h, spacing, split_point_fn, ratio_fn)
        _recursive_partition(rects, cancelled, sorted_ar, ar_prefix, inv_prefix, start + left_count, count - left_count, x + left_w + spacing, y, right_w, h, spacing, split_point_fn, ratio_fn)
    else:
        top_h = max(1, int(h * ratio) - spacing // 2)
        bottom_h = max(1, h - top_h - spacing)
        _recursive_partition(rects, cancelled, sorted_ar, ar_prefix, inv_prefix, start, left_count, x, y, w, top_h, spacing, split_point_fn, ratio_fn)
        _recursive_partition(rects, cancelled, sorted_ar, ar_prefix, inv_prefix, start + left_count, count - left_count, x, y + top_h + spacing, w, bottom_h, spacing, split_point_fn, ratio_fn)


def ar_weighted_ratio(sorted_ar, ar_prefix, inv_prefix, start, count, left_count, split_x):
    if split_x:
...
def equal_ratio(sorted_ar, ar_prefix, inv_prefix, start, count, left_count, split_x):
...
class PartitionCalculatorBase(BaseLayoutCalculator):
    ...
    def _area_ratio(self, sorted_ar, ar_prefix, inv_prefix, start, count, left_count, split_x):
        raise NotImplementedError
    ...
    def _calculate(self):
        ...
        _recursive_partition(sorted_rects, lambda: self._cancelled, sorted_ar, ar_prefix, inv_prefix, 0, n, 0, 0, pw, ph, spacing, self._split_point, self._area_ratio)
