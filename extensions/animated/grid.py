...
class AnimatedGridPlugin(WidgetGridPlugin):
    ...
    def render(self, widget, path, size):
        ...
        widget._path = path
        self._dispatcher.post(lambda: self._decode_and_set(widget, path, size, cancel), priority=0, cancel=cancel)
    ...
    def _decode_and_set(self, widget, path, size, cancel):
        ...
        self._dispatcher.invoke(lambda: widget.set_frames(path, frames, delays) if widget._path == path else None)
