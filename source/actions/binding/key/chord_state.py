...
class KeyChordStateManager:
    ...
    def dedupe(self, stamp: KeyEventStamp) -> bool:
        return self._deduper.add_and_check((stamp.et, stamp.sc, stamp.key, stamp.ts, stamp.wid))
    ...
    def remove_physical_press(self, sc: int):
        ...
        self._physical.unconsume(int(sc))
    ...
    def remove_logical_press(self, key: int):
        ...
        self._logical.unconsume(int(key))
    ...
    def map_sc_to_key(self, sc: int, key: int) -> int:
        return int(self._sc_mapper.map(0, int(sc), int(key)))
