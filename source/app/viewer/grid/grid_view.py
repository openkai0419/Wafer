...
from source.core.qt.rate_limit import qt_debounce, qt_throttle
from source.core.qt.pixmap import PixmapFactory
...
class GridView(QtWidgets.QGraphicsView, ActionKit.UIMixin):
    ...
    def __init__(self, root, items: GridItemModel | None = None, parent=None):
        ...
        self.base_height = app_settings.get('viewer/zoom', int(self.screen_width / 10))
        self._width_ref = 0
        self.min_height = int(self.screen_width / 30)
        self.max_height = int(self.screen_width)
        self.setMinimumWidth(self.min_height)
        self.spacing = dpix(4)
        self.orientation = app_settings.get('viewer/orientation', 0)
        ...
        self.proxy_pool = ProxyWidgetPool(self._scene, grid_resolver)
        self.active_loaders = {}
    ...
    def set_orientation(self, orientation):
        ...
        app_settings.set('viewer/orientation', orientation)
        self._setup_primary_scroll()
        self._recalc_layout()
    ...
    def _is_center_anchor(self):
        from source.core.actions.bridge import Command
    ...
    def get_restore_index(self):
        if app_settings.check_and_mark_seen('viewer/scroll'):
            return app_settings.get('viewer/scroll', 0)
    ...
    def _on_widget_ready(self, index, plugin_name):
        ...
        if proxy is not None:
            ...
            grid_resolver.render(path, proxy.widget(), size)