...
def call0(obj: Any, name: str) -> Any:
    m = get_callable(obj, name)
    return m() if m is not None else None


def call_int0(obj: Any, name: str, default: int = 0) -> int:
    v = call0(obj, name)
    if v is None:
        return int(default)
...
def try_call0(obj: Any, name: str, default: Any = None, warn_text: str | None = None) -> Any:
    m = get_callable(obj, name)
    if m is None:
        return default
    try:
