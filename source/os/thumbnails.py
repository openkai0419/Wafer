...
class FileThumbnailer:
    def __init__(self):
        ...
        if self.platform.startswith("win"):
            ...
            self._shell32.SHCreateItemFromParsingName.argtypes = [
    ...
    def _get_thumbnail_windows(self, file_path, size):
        ...
        handle = c_void_p()
        hr = self._shell32.SHCreateItemFromParsingName(
            file_path, None,
            byref(self._IShellItemImageFactory._iid_), byref(handle))
        if hr != 0:
        ...
        hbitmap = factory.GetImage(SIZE(size, size), 0x0)
        if not hbitmap:
    ...
    def _get_thumbnail_mac(self, file_path, size):
        try:
            ...
            thumb = self._QLThumbnailImageCreate(
                self._kCFAllocatorDefault, url, 
                self._CGSizeMake(size, size), None)
            if thumb:
