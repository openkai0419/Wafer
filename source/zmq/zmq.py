...
class MessageEnvelope:
    def __init__(
        ...
        self._topic_b = _topic_b
        self._msg_b = _msg_b
        self.request_id = request_id
        self._app_s = _app_s
        self._target_s = _target_s
        self._table_s = _table_s
        self._topic_s = _topic_s
        self._msg_s = _msg_s
...
class ZMQBroker:
    ...
    def _pick_broadcast_targets(self, app_b, target_b):
        with self._lock:
            if target_b == b'all':
    ...
    def _handle_control(self, ident, payload):
        ...
        if text.startswith('get_count:'):
            ...
            reply = MessageEnvelope.build(APP_NAME=parts[3] if len(parts) > 3 else APP_FILE_NAME, targetprocess='ALL', topic='control.count.reply', message=counts_str, request_id=req_id)
