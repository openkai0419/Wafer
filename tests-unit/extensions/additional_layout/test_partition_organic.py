...
def test_index_spatial_coherence():
    aspects = [2.0, 0.5, 1.5, 0.8, 1.0, 0.3, 2.5, 1.2, 0.6, 1.0, 1.5, 0.7, 2.0, 0.4, 1.3, 0.9, 1.1, 0.5, 2.0, 1.0]
    calc = OrganicPartitionCalculator(aspects, 100, 5, 800, 800, 0)
    calc.run()
    layout = calc._result
    for i in range(len(layout) - 1):
        dist = abs(layout[i].y() - layout[i + 1].y())
        assert dist < layout.total_extent, f"Items {i} and {i + 1} are suspiciously far apart"
