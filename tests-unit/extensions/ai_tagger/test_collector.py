...
class TestPostInstall:
    def test_post_install_calls_ensure_model(self):
        with patch("wafer.plugin.installer.install_packages", return_value=True), patch("extensions.ai_tagger.collector.ensure_model") as mock_model, patch.object(WD14TaggerCollector, "_verify_gpu_provider"):
            WD14TaggerCollector.post_install("/fake/dir")
            mock_model.assert_called_once()

    def test_post_install_tries_gpu_first(self):
        with patch("extensions.ai_tagger.collector.ensure_model"), patch("wafer.plugin.installer.install_packages", return_value=True) as mock_install, patch.object(WD14TaggerCollector, "_verify_gpu_provider"):
            WD14TaggerCollector.post_install("/fake/dir")
    ...
    def test_post_install_falls_back_to_cpu(self):
        ...
        with patch("extensions.ai_tagger.collector.ensure_model"), patch("wafer.plugin.installer.install_packages", side_effect=side_effect) as mock_install, patch.object(WD14TaggerCollector, "_verify_gpu_provider"):
            WD14TaggerCollector.post_install("/fake/dir")
