...
class TestSafeDownload:
    def _patch_urlopen(self, monkeypatch, payload: bytes):
        monkeypatch.setattr(dl.urllib.request, "urlopen", lambda req, timeout=None: _FakeResp(payload))
    ...
    def test_sha256_match(self, tmp_path, monkeypatch):
        self._patch_urlopen(monkeypatch, HELLO)
        dest = str(tmp_path / "out.bin")
        dl.safe_download("https://example.com/x", dest, allowed_hosts=("example.com",), expected_sha256=HELLO_SHA)
    ...
    def test_sha256_mismatch_raises_and_cleans(self, tmp_path, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="SHA256 mismatch"):
            dl.safe_download("https://example.com/x", dest, allowed_hosts=("example.com",), expected_sha256="0" * 64)
    ...
    def test_on_progress_reports_done_and_total(self, tmp_path, monkeypatch):
        self._patch_urlopen(monkeypatch, HELLO)
        progress = []
        dl.safe_download("https://example.com/x", str(tmp_path / "out.bin"), allowed_hosts=("example.com",), on_progress=lambda done, total: progress.append((done, total)))
    ...
    def test_user_agent_header_sent(self, tmp_path, monkeypatch):
        ...
        dl.safe_download("https://example.com/x", str(tmp_path / "out.bin"), allowed_hosts=("example.com",), user_agent="ua-test")
    ...
    def test_download_failure_cleans_tmp(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(dl.urllib.request, "urlopen", lambda req, timeout=None: FailingResp(b"partial"))
        dest = str(tmp_path / "out.bin")
    ...
    def test_disallowed_host_rejected(self, tmp_path, monkeypatch):
        called = []
        monkeypatch.setattr(dl.urllib.request, "urlopen", lambda *a, **k: called.append(a))
        with pytest.raises(ValueError):
            dl.safe_download("https://evil.com/x", str(tmp_path / "x"), allowed_hosts=("example.com",))
        assert called == []
...
class TestFetchText:
    def test_success(self, monkeypatch):
        ...
        monkeypatch.setattr(dl.urllib.request, "urlopen", fake)
        out = dl.fetch_text("https://example.com/x", allowed_hosts=("example.com",), max_bytes=1024, user_agent="ua-test")
        assert out == "hello"
    ...
    def test_extra_headers_passed(self, monkeypatch):
        ...
        monkeypatch.setattr(dl.urllib.request, "urlopen", fake)
        dl.fetch_text("https://example.com/x", allowed_hosts=("example.com",), max_bytes=64, user_agent="u", extra_headers={"Accept": "application/json"})
    ...
    def test_max_bytes_exceeded(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="too large"):
            dl.fetch_text("https://example.com/x", allowed_hosts=("example.com",), max_bytes=10, user_agent="u")


class TestFetchJson:
    def test_parses(self, monkeypatch):
        monkeypatch.setattr(dl.urllib.request, "urlopen", lambda req, timeout=None: _FakeResp(json.dumps({"a": 1}).encode()))
        assert dl.fetch_json("https://example.com/x", allowed_hosts=("example.com",), max_bytes=64, user_agent="u") == {"a": 1}
...
class TestValidateArchivePath:
    ...
    def test_absolute_rejected(self, tmp_path):
        with pytest.raises(ValueError, match="traversal"):
            dl.validate_archive_path("C:\\Windows\\evil" if os.name == "nt" else "/etc/passwd", str(tmp_path))
...
class TestExtract7zMembers:
    def test_py7zr_path_flat_target(self, tmp_path, monkeypatch):
        ...
        target = tmp_path / "lib"
        dl.extract_7z_members(str(tmp_path / "fake.7z"), str(target), ("ffprobe.exe", "ffmpeg.exe"))
    ...
    def test_archive_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="no 7zr"):
            dl.extract_7z_members(str(tmp_path / "fake.7z"), str(tmp_path / "out"), ("evil.exe",))
