...
def _get(db, fh, key):
    ...
    row = cur.execute("SELECT value, locked FROM tags WHERE file_hash=? AND key=?", (fh, key)).fetchone()
    cur.close()
    return row
...
def test_lock_only_does_not_change_value(tmp_path):
    ...
    _seed(db)
    _apply(db, "src1", [("rating", "5", 5.0, 0)], [])
    fh, applied, deleted = _apply(db, "src1", [("rating", "ignored", None, 1)], [], lock_only=True)
...
def test_lock_only_skips_missing_keys(tmp_path):
    db = _make_db(tmp_path)
    _seed(db)
    fh, applied, deleted = _apply(db, "src1", [("ghost", "x", None, 1)], [], lock_only=True)
    assert applied == []
...
def test_locked_delete_returns_empty_deleted(tmp_path):
    db = _make_db(tmp_path)
    _seed(db)
    _apply(db, "src1", [("k1", "v1", None, 1)], [])
    fh, applied, deleted = _apply(db, "src1", [], ["k1"])
    assert deleted == []
...
def test_migration_carries_lock_flag(tmp_path):
    db = _make_db(tmp_path)
    _seed(db)
    _apply(db, "src1", [("user", "U", None, 1), ("free", "F", None, 0)], [])
