from wafer.core.store.workspace import (
    BarSpec,
    PathPreset,
    QueryPreset,
    UIPreset,
...
class TestPresetRoundtrip:
    ...
    def test_query_preset_roundtrip(self, tmp_path):
        store = WorkspaceStore(path=str(tmp_path / "ws.json"))
        bars = [BarSpec(filter="text", params={"keywords": "cat"}, op=None, enabled=True), BarSpec(filter="text", params={"keywords": "dog"}, op="OR", enabled=True)]
