...
from wafer.core.profiling import MemoryWatchdog, profiler
...
def test_compile():
    py_compile.compile("wafer/core/profiling.py")
...
def test_record_elapsed_uses_perf_counter(monkeypatch):
    profiler.set_enabled(True)
    monkeypatch.setattr("wafer.core.profiling.time.perf_counter", lambda: 10.5)
...
def test_wrap_queued_records_wait(monkeypatch):
    ...
    monkeypatch.setattr("wafer.core.profiling.time.perf_counter", lambda: next(moments))
    called = []
...
def test_memwatch_report_logs_rss_and_delta(monkeypatch):
    ...
    logs = []
    monkeypatch.setattr("wafer.core.profiling.AppLogger.info", lambda text: logs.append(text))
