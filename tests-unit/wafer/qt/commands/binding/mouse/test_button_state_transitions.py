...
from wafer.qt.commands.binding.mouse.types import (
    ClickType,
    MouseActionKey,
    MouseButton,
)
from wafer.qt.commands.binding.mouse.manager import (
    MouseEventManager,
...
class TestModifiers:
    def test_ctrl_left_click(self, qtbot):
        from wafer.qt.commands.binding.mouse.types import ModifierKey
...
class TestStateCleanupAfterSequence:
    ...
    def test_clean_after_right_hold_left(self, qtbot):
        self._run_sequence_and_check_clean(
            qtbot,
            [
                P(RMB),
                P(LMB, RMB),
