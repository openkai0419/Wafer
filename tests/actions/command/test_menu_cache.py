...
from source.actions.command.core import (
    CommandBase,
    CommandMeta,
    CommandRegistry,
    register_command_defs,
)
from source.actions.command.ui import CommandMenuBuilder, MenuBuilder
from source.actions.command.maker import MenuMaker
from source.actions.command.state import ActionGroupStateManager, CommandOptionStore
...
class TestExternalStateChangeBugs:
    def test_individual_check_stale_after_external_store_change(self, qtbot):
        ...
        builder.refresh_check_states(menu)
        assert wa.isChecked() is False, (
            "_check_states shadows CommandOptionStore: "
            "external change not reflected"
        )
        assert container._chk.text() == ""
    ...
    def test_group_cycle_action_group_reflects(self, qtbot):
        ...
        builder.refresh_check_states(menu)
        assert wa_a.isChecked() is False, (
            "After cycle, old item should be unchecked"
        )
        assert wa_b.isChecked() is True, (
            "After cycle, new item should be checked"
        )
