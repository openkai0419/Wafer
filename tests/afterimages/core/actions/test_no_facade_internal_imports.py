...
def _iter_py_files() -> list[Path]:
    root = Path('afterimages/core/actions')
    out: list[Path] = []
...
def _is_forbidden_facade_import(module: str | None, level: int, name: str | None) -> bool:
    if module is None:
        return False
    if module == 'afterimages.core.actions.bridge':
        return True
        ...
        for n in ast.walk(tree):
            if isinstance(n, ast.Import):
                for alias in n.names:
                    if alias.name == 'afterimages.core.actions.bridge':
                        offenders.append(str(p))
            elif isinstance(n, ast.ImportFrom):
                if _is_forbidden_facade_import(n.module, n.level, None):
                    offenders.append(str(p))
    offenders = sorted(set(offenders))
    assert not offenders, 'bridge.py must not be imported from inside afterimages/actions: ' + ', '.join(offenders)
