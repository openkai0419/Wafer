...
def _multispan_ratio(layout, n, cell_w):
    threshold = cell_w + 5
    count = sum(1 for i in range(n) if layout[i].width() > threshold or layout[i].height() > threshold)
...
def test_benchmark_speed(n):
    ...
    print(f"\n[MultiSpan] n={n:>5d}  best={best:.4f}s  avg={avg:.4f}s  per_item={avg / n * 1e6:.1f}us")
