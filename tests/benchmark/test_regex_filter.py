...
def approach_a_post_filter_meta(conn, pattern, keys):
    ...
    if keys:
        ...
        meta_rows = conn.execute(f"SELECT path, value FROM meta_info WHERE key IN ({placeholders})", keys).fetchall()
        for r in meta_rows:
...
def approach_b_like_then_post_filter(conn, pattern, like_hint):
    ...
    rows = conn.execute("SELECT path FROM files WHERE path LIKE ? ESCAPE '\\'", (f"%{esc}%",)).fetchall()
    return [r["path"] for r in rows if compiled.search(r["path"])]
...
def approach_b_like_then_post_filter_meta(conn, pattern, like_hint, keys):
    ...
    if keys:
        placeholders = ",".join("?" for _ in keys)
        meta_rows = conn.execute(f"SELECT path, value FROM meta_info WHERE key IN ({placeholders}) AND value LIKE ? ESCAPE '\\'", [*keys, f"%{esc}%"]).fetchall()
        for r in meta_rows:
            if compiled.search(r["value"] or ""):
                matched_paths.add(r["path"])
...
def approach_c_sql_regexp(conn, pattern):
    rows = conn.execute("SELECT DISTINCT path FROM files WHERE path REGEXP ?", (pattern,)).fetchall()
...
def approach_c_sql_regexp_meta(conn, pattern, keys):
    ...
    if keys:
        placeholders = ",".join("?" for _ in keys)
        parts.append(f"SELECT path FROM meta_info WHERE key IN ({placeholders}) AND value REGEXP ?")
        params.extend(keys)
        params.append(pattern)
        parts.append(
...
class TestRegexBenchmark:
    def test_path_only(self, tmp_path, n):
        ...
        assert set(res_a) == set(res_b) == set(res_c), f"Result mismatch: A={len(res_a)}, B={len(res_b)}, C={len(res_c)}"
