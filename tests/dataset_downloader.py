...
_VIDEO_SOURCES = [
    ("BigBuckBunny_320x180.mp4", "https://download.blender.org/peach/bigbuckbunny_movies/BigBuckBunny_320x180.mp4", "CC BY 3.0"),
    ("sintel_trailer-480p.mp4", "https://download.blender.org/durian/trailer/sintel_trailer-480p.mp4", "CC BY 3.0"),
    ("big_buck_bunny_720p_h264.mov", "https://download.blender.org/peach/bigbuckbunny_movies/big_buck_bunny_720p_h264.mov", "CC BY 3.0"),
    ("sintel_trailer-720p.mp4", "https://download.blender.org/durian/trailer/sintel_trailer-720p.mp4", "CC BY 3.0"),
    ("big_buck_bunny_1080p_h264.mov", "https://download.blender.org/peach/bigbuckbunny_movies/big_buck_bunny_1080p_h264.mov", "CC BY 3.0"),
]
...
def _is_trusted_host(url: str) -> bool:
    ...
    if not hostname:
        return False
    return any(hostname == s or hostname.endswith("." + s) for s in _ALLOWED_HOST_SUFFIXES)
...
def _generate_animated_gif(path: Path, w: int, h: int, n_frames: int, seed: int):
    ...
    shape_colors = [(rng.randint(100, 255), rng.randint(100, 255), rng.randint(100, 255)) for _ in range(3)]
    shapes = []
...
def _generate_apng(path: Path, w: int, h: int, n_frames: int, seed: int):
    ...
    for i in range(n_frames):
        ...
        fctl_data = struct.pack(">IIIIIHHBB", seq_num, w, h, 0, 0, 80, 1000, 0, 0)
...
def main():
    ...
    parser.add_argument("--dest", default=str(DEFAULT_DEST), help="Download destination (default: .sample/)")
    sub = parser.add_subparsers(dest="command", required=True)

    dl = sub.add_parser("download", help="Download / generate sample files")
    dl.add_argument("--preset", default="standard", choices=PRESETS.keys(), help="Preset configuration (default: standard)")
    dl.add_argument("--images", type=int, default=None, help="Override image count")
    dl.add_argument("--videos", type=int, default=None, help="Override video count")
    dl.add_argument("--animated", type=int, default=None, help="Override animated count")
