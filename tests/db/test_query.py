...
class TestMetaQuerySubquery:

    def test_keys_meta_info(self, populated_db):
        q = MetaQuery(keys=["dpi"])
    ...
    def test_no_keys_require_keys_true(self):
        q = MetaQuery(keys=None, require_keys=True)
        sql, params = q._make_subquery(np)
        assert sql is None
    ...
    def test_keyword_include(self, populated_db):
        q = MetaQuery(keys=["Comment"], keywords="photo", query_mode="LIKE")
        sql, params = q._make_subquery(np)
        assert sql is not None
        assert "LIKE" in sql

    def test_keyword_exclude(self, populated_db):
        q = MetaQuery(keys=["Comment"], keywords="-badword", splittext=",")
        sql, params = q._make_subquery(np)
        assert sql is not None
        assert "NOT IN" in sql

    def test_directory_filter(self, populated_db):
        q = MetaQuery(keys=["dpi"], directories=["C:/photos/vacation"])
        sql, params = q._make_subquery(np)
        assert sql is not None
        assert "LIKE" in sql

    def test_directory_no_subfolders(self, populated_db):
        q = MetaQuery(keys=["dpi"], directories=["C:/photos/vacation"], include_subfolders=False)
        sql, params = q._make_subquery(np)
        assert sql is not None
        assert "NOT LIKE" in sql

    def test_empty_keyword_filtered(self):
        q = MetaQuery(keys=["__filepath__"], keywords="", splittext=",")
    ...
    def test_mixed_empty_keywords(self):
        q = MetaQuery(keys=["__filepath__"], keywords="a,,b,-,-c", splittext=",")
        keys, include, exclude = q.normalize_inputs()
        assert include == ["a", "b"]
        assert exclude == ["c"]


class TestMetaInfoSearchEngineGet:

    def test_get_by_meta_key(self, populated_db):
        engine = MetaInfoSearchEngine(populated_db)
        paths, sources, aspects = engine.get(MetaQuery(keys=["dpi"]))
        assert len(paths) == 200

    def test_get_by_filepath_keyword(self, populated_db):
        engine = MetaInfoSearchEngine(populated_db)
        paths, sources, aspects = engine.get(MetaQuery(keys=["__filepath__"], keywords="vacation"))
        assert all("vacation" in p for p in paths)
        assert len(paths) == 100

    def test_get_by_meta_keyword(self, populated_db):
        engine = MetaInfoSearchEngine(populated_db)
        paths, sources, aspects = engine.get(MetaQuery(keys=["Comment"], keywords="photo number 5", query_mode="LIKE"))
        assert len(paths) > 0

    def test_get_with_directory(self, populated_db):
        engine = MetaInfoSearchEngine(populated_db)
        paths, sources, aspects = engine.get(MetaQuery(keys=["dpi"], directories=["C:/photos/vacation"]))
        assert all("vacation" in p for p in paths)
        assert len(paths) == 100

    def test_get_with_directory_no_subfolders(self, populated_db):
        engine = MetaInfoSearchEngine(populated_db)
        paths, sources, aspects = engine.get(
            MetaQuery(keys=["dpi"], directories=["C:/photos"], include_subfolders=False)
        )
        assert len(paths) == 0

    def test_get_with_exclude(self, populated_db):
        engine = MetaInfoSearchEngine(populated_db)
        all_paths, _, _ = engine.get(MetaQuery(keys=["Comment"]))
        excluded_paths, _, _ = engine.get(MetaQuery(keys=["Comment"], keywords="-number 0,", splittext=","))
        assert len(excluded_paths) < len(all_paths)

    def test_get_sort_by_name(self, populated_db):
        engine = MetaInfoSearchEngine(populated_db)
        paths, _, _ = engine.get(MetaQuery(keys=["dpi"], sort_by="name", ascending=True))
        names = [os.path.basename(p) for p in paths]
        assert names == sorted(names)

    def test_get_sort_by_name_desc(self, populated_db):
        engine = MetaInfoSearchEngine(populated_db)
        paths, _, _ = engine.get(MetaQuery(keys=["dpi"], sort_by="name", ascending=False))
        names = [os.path.basename(p) for p in paths]
        assert names == sorted(names, reverse=True)

    def test_get_random(self, populated_db):
        engine = MetaInfoSearchEngine(populated_db)
        paths, _, _ = engine.get(MetaQuery(keys=["dpi"], sort_by="random"))
        assert len(paths) == 200

    def test_get_no_results(self, populated_db):
        engine = MetaInfoSearchEngine(populated_db)
        paths, sources, aspects = engine.get(MetaQuery(keys=["nonexistent_key"]))
        assert paths == []

    def test_get_glob_mode(self, populated_db):
        engine = MetaInfoSearchEngine(populated_db)
        paths, _, _ = engine.get(MetaQuery(keys=["__filepath__"], keywords="*vacation*", query_mode="GLOB"))
        assert len(paths) == 100


class TestMetaInfoSearchEngineListKeys:

    def test_list_all_keys(self, populated_db):
        engine = MetaInfoSearchEngine(populated_db)
        q = MetaQuery(directories=["C:/photos/vacation"])
    ...
    def test_list_all_keys_dedup(self, db_path):
        ...
        db.conn.commit()
        db.exit()
        engine = MetaInfoSearchEngine(db_path)
        q = MetaQuery()
        keys = engine.list_all_keys(q, sort_by_freq=True)
...
class TestMetaInfoSearchEngineCombined:

    def test_get_combined_union(self, populated_db):
        engine = MetaInfoSearchEngine(populated_db)
        q1 = MetaQuery(keys=["__filepath__"], keywords="vacation", append_mode="OR")
        q2 = MetaQuery(keys=["__filepath__"], keywords="work", append_mode="OR")
        paths, aspects = engine.get_combined([q1, q2])
        assert len(paths) == 200

    def test_get_combined_intersect(self, populated_db):
        engine = MetaInfoSearchEngine(populated_db)
        q1 = MetaQuery(keys=["dpi"], append_mode="OR")
        q2 = MetaQuery(keys=["__filepath__"], keywords="vacation", append_mode="AND")
        paths, aspects = engine.get_combined([q1, q2])
        assert all("vacation" in p for p in paths)
...
class TestExplainQueryPlanRealDB:
    ...
    def real_engine(self):
        ...
        return MetaInfoSearchEngine(self.REAL_DB)
    ...
    def test_filepath_query(self, real_engine):
        q = MetaQuery(keys=["__filepath__"], keywords="img")
        subq, params = q._make_subquery(np)
        sql = f"""
            SELECT m.path, m.source, m.aspect_ratio
            FROM images_full AS m
    ...
    def test_directory_query(self, real_engine):
        ...
        q = MetaQuery(keys=["__filepath__"], directories=[sample_dir])
        subq, params = q._make_subquery(np)
        sql = f"""
            SELECT m.path, m.source, m.aspect_ratio
            FROM images_full AS m
    ...
    def test_list_all_keys_query(self, real_engine):
        ...
        parts = sample[0].replace("\\", "/").split("/")
        sample_dir = "/".join(parts[:4])
        q = MetaQuery(directories=[sample_dir])
