...
class TestBuildPathQuery:
    ...
    def test_preset_with_date_ref(self):
        params = {
            'target_key': 'modified', 'mode': 'preset',
            'preset_value': 7, 'preset_unit': 'days',
    ...
    def test_range_swaps_min_max(self):
        params = {
            'target_key': 'modified', 'mode': 'range',
            'range_from': '2024/12/31', 'range_to': '2024/01/01',
        }
        sql, bind = DateRangeFilter.build_path_query(params, lambda p: p)
        assert 'BETWEEN' in sql
        assert bind[1] < bind[2]
...
class TestQueryExecution:
    ...
    def test_range_filters_between(self, tmp_path):
        ...
        result = _query(db, {
            'target_key': 'modified', 'mode': 'range',
            'range_from': '2024/05/01', 'range_to': '2024/08/01',
        })
    ...
    def test_exif_datetime_key(self, tmp_path):
        ...
        db = _setup_db(tmp_path, files=files, meta_num=meta)
        result = _query(db, {
            'target_key': 'exif.DateTimeOriginal', 'mode': 'range',
            'range_from': '2024/01/01', 'range_to': '2024/12/31',
        })
    ...
    def test_null_value_num_excluded(self, tmp_path):
        ...
        db = _setup_db(tmp_path, files=files, meta_num=meta)
        result = _query(db, {
            'target_key': 'exif.LensMake', 'mode': 'preset',
            'preset_value': 7, 'preset_unit': 'days',
        })
        assert len(result) == 0
