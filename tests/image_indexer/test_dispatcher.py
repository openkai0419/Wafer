...
from source.image_indexer.dispatcher import CollectorDispatcher, _BATCH_SIZE, _DISPATCH_INTERVAL


def test_compile():
    py_compile.compile('source/image_indexer/dispatcher.py')
...
def test_init_with_defaults(tmp_path):
    ...
    assert 'image' in dispatcher._collectors


def test_init_with_custom_collectors(tmp_path):
    from source.db.indexer import FileIndexer
    idx = FileIndexer(tmp_path / 'test.db')
    dispatcher = CollectorDispatcher('testdb', idx, collectors=['image', 'video'])
    assert dispatcher._collectors == ['image', 'video']
...
def test_reset_stale_on_start(tmp_path):
    ...
    with FileIndexer(db_path) as idx:
        ...
        idx.db.insert_pending_collection(['src1'], ['image'])
        idx.db.mark_dispatched(['src1'], 'image')
