...
class TestIndexToSearch:
    ...
    def test_indexed_files_searchable_by_name(self, tmp_path):
        db_path, img_dir, paths = _build_populated_db(tmp_path, [
    ...
    def test_collected_metadata_searchable(self, tmp_path):
        ...
        meta_rows = db.read_conn.execute(
            "SELECT key FROM meta_info WHERE path=?", (norm,)
        ).fetchall()
        db.close()

        if meta_rows:
            key_name = meta_rows[0][0]
            all_paths, _, _ = engine.search(SearchQuery(
                keys=(key_name,),
                require_keys=True,
            ))
    ...
    def test_directory_filter_after_index(self, tmp_path):
        ...
        filtered, _, _ = engine.search(SearchQuery(
            directories=(norm_a,),
            require_keys=False,
        ))
    ...
    def test_sort_order_reflects_db_state(self, tmp_path):
        ...
        asc_paths, _, _ = engine.search(SearchQuery(
            sort_by='name', ascending=True, require_keys=False,
        ))
        names_asc = [os.path.basename(p) for p in asc_paths]
        assert names_asc == sorted(names_asc)

        desc_paths, _, _ = engine.search(SearchQuery(
            sort_by='name', ascending=False, require_keys=False,
        ))
    ...
    def test_aspect_ratio_reflects_image_dimensions(self, tmp_path):
        db_path, _, paths = _build_populated_db(tmp_path, [
        ...
        engine = FileSearchEngine(str(db_path))
        result_paths, _, aspects = engine.search(SearchQuery(
            sort_by='name', ascending=True, require_keys=False,
        ))
