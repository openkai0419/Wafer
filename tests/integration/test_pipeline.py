...
class TestImagePipeline:
    def test_full_image_pipeline(self, tmp_path):
        ...
        with FileIndexer(db_path, collectors=collectors) as idx:
            ...
            db.start()
            try:
                src_row = db.read_conn.execute("SELECT source FROM sources WHERE source=?", (norm,)).fetchone()
                assert src_row is not None

                file_row = db.read_conn.execute("SELECT aspect_ratio FROM files WHERE path=?", (norm,)).fetchone()
                assert file_row[0] == 2.0

                meta = db.read_conn.execute("SELECT key, value FROM meta_info WHERE path=?", (norm,)).fetchall()
                assert len(meta) > 0
        ...
        with FileIndexer(db_path, collectors=collectors) as idx:
            ...
            try:
                for img_p in [jpg_path, png_path]:
                    ...
                    assert cs[0] == "ok"
                    src = db.read_conn.execute("SELECT source FROM sources WHERE source=?", (norm,)).fetchone()
                    assert src is not None

                txt_norm = normalize_path(str(txt_path))
                txt_src = db.read_conn.execute("SELECT source FROM sources WHERE source=?", (txt_norm,)).fetchone()
    ...
    def test_modified_file_re_registered(self, tmp_path):
        ...
        old_mtime = db_check.read_conn.execute("SELECT modified FROM sources WHERE source=?", (norm,)).fetchone()[0]
        db_check.close()
...
class TestDispatcherSimulation:
    def test_pending_to_dispatched_to_ok(self, tmp_path):
        ...
        with FileIndexer(db_path, collectors=collectors) as idx:
            ...
            dispatched_count = idx.db.read_conn.execute("SELECT count(*) FROM collection_status WHERE collector='exif' AND status='dispatched'").fetchone()[0]
            ...
            try:
                ...
                for p in paths:
                    ...
                    row = db.read_conn.execute("SELECT source FROM sources WHERE source=?", (norm,)).fetchone()
...
class TestWriterCollectorField:
    def test_collector_name_from_payload(self, tmp_path):
        ...
        db.conn.execute("INSERT INTO sources (source, file_hash, size, modified) VALUES ('src1', 'h1', 100, 1.0)")
        db.conn.execute("INSERT INTO files (path, source, aspect_ratio) VALUES ('src1', 'src1', 1.0)")
        db.conn.commit()
        db.close()
        ...
        db2 = FileDB(db_path)
        db2.start()
        cs = db2.read_conn.execute("SELECT collector, status FROM collection_status WHERE source='src1'").fetchone()
