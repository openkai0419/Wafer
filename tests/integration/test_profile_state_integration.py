...
class TestSessionAndStateStoreCombined:
    def test_session_stores_state_data(self, tmp_path):
        ...
        session_store = ProfileStore(str(tmp_path / "profiles.json"))
        sid = session_store.create_profile("Test Profile")
        entry = session_store.get_profile(sid)
        ...
        session_store.save_profile(entry)

        loaded = session_store.get_profile(sid)
    ...
    def test_session_restore_feeds_state_store(self, tmp_path):
        session_store = ProfileStore(str(tmp_path / "profiles.json"))
        sid = session_store.create_profile("Restore Test")
        entry = session_store.get_profile(sid)
    ...
    def test_deferred_state_restore(self, tmp_path):
        session_store = ProfileStore(str(tmp_path / "profiles.json"))
        sid = session_store.create_profile("Deferred")
        entry = session_store.get_profile(sid)
        entry.ui = UIState(
            component_states={
                "panel_a": {"width": 250},
...
class TestSessionWithSearchParams:
    def test_full_search_params_roundtrip(self, tmp_path):
        ...
        sid = store.create_profile("Full Search")
    ...
    def test_empty_search_params(self, tmp_path):
        store = ProfileStore(str(tmp_path / "profiles.json"))
        sid = store.create_profile("Empty Search")
        entry = store.get_profile(sid)
        entry.query_snapshot = QueryState()
        store.save_profile(entry)
...
class TestMultiSessionStateIsolation:
    def test_two_sessions_different_states(self, tmp_path):
        ...
        loaded_a = store.get_profile(sid_a)
        loaded_b = store.get_profile(sid_b)
    ...
    def test_session_a_restore_does_not_affect_b(self, tmp_path):
        store = ProfileStore(str(tmp_path / "profiles.json"))
        sid_a = store.create_profile("A")
        sid_b = store.create_profile("B")
...
class TestBookmarkWithSession:
    def test_bookmark_and_session_share_query_format(self, tmp_path):
        ...
        sid = session_store.create_profile("From Bookmark")
        entry = session_store.get_profile(sid)
        ...
        loaded_session = session_store.get_profile(sid)
