...
def test_paste_executor_overwrite_same_path_is_noop(tmp_path):
    from source.core.platform.file_operations import PasteDecision, PasteExecutor, PastePlanItem
...
def test_paste_executor_copy_dir_rename_on_conflict(tmp_path):
    from source.core.platform.file_operations import PasteDecision, PasteExecutor, PastePlanItem, unique_path
...
def test_scan_merge_conflicts_detects_files(tmp_path):
    from source.core.platform.file_operations import scan_merge_conflicts
...
def test_unique_path_generates_increment(tmp_path):
    from source.core.platform.file_operations import unique_path
...
def test_check_copy_conflict_same_path(tmp_path):
    from source.core.platform.file_operations import check_copy_conflict
...
def test_sanitize_filename_windows_invalid_chars():
    from source.core.platform.file_operations import sanitize_filename

    assert sanitize_filename("a<b>c.txt") == "a_b_c.txt"


def test_execute_paste_plans_with_ui_exists():
    from source.core.platform.file_operations import execute_paste_plans_with_ui
    assert callable(execute_paste_plans_with_ui)


def test_drop_files_with_ui_exists():
    from source.core.platform.file_operations import drop_files_with_ui
    assert callable(drop_files_with_ui)


def test_paste_clipboard_files_exists():
    from source.core.platform.file_operations import paste_clipboard_files
...
def test_paste_cancelled_error_importable():
    from source.core.platform.file_operations import PasteCancelledError
