...
from source.core.zmq.transport import Priority
from source.core.zmq.message import Message
from source.core.zmq.node import Node
from source.core.zmq.outbox import OutboxStore, _extract_pid, _delete_db_files


class TestOutboxStore:

    def _make_store(self, tmp_dir, node_id='test-node'):
        with patch('source.core.zmq.outbox._OUTBOX_DIR', tmp_dir):
    ...
    def test_scan_all(self):
        with tempfile.TemporaryDirectory() as tmp:
            with patch('source.core.zmq.outbox._OUTBOX_DIR', tmp):
    ...
    def test_cleanup_empty_files_removes_dead_pid(self):
        with tempfile.TemporaryDirectory() as tmp:
            with patch('source.core.zmq.outbox._OUTBOX_DIR', tmp):
                ...
                with patch('source.core.zmq.outbox.psutil.pid_exists', return_value=False):
    ...
    def test_cleanup_empty_files_keeps_alive_pid(self):
        with tempfile.TemporaryDirectory() as tmp:
            with patch('source.core.zmq.outbox._OUTBOX_DIR', tmp):
                ...
                with patch('source.core.zmq.outbox.psutil.pid_exists', return_value=True):
                    OutboxStore.cleanup_empty_files()
                assert os.path.exists(db_path)
