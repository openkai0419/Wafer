...
from source.core.zmq.transport import NODE_TIMEOUT, Priority
from source.core.zmq.broker import Broker, _CoalescingQueue, _PeerInfo
from source.core.zmq.message import Message
from source.core.zmq.node import Node


def test_compile():
    py_compile.compile('source/core/zmq/message.py')
    py_compile.compile('source/core/zmq/broker.py')
    py_compile.compile('source/core/zmq/node.py')
    py_compile.compile('source/core/zmq/transport.py')
    py_compile.compile('source/core/zmq/outbox.py')
...
class TestReconnection:
    ...
    def test_node_reconnects_after_broker_restart(self):
        ...
        with patch('source.core.zmq.node.read_broker_port') as mock_read:
            broker2 = Broker()
            broker2.start()
