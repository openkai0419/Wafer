...
class TestTextFilterBuildPathQuery:
    ...
    def test_exclude_keywords(self, engine):
        params = {
            'keys': ['Comment'], 'keywords': 'photo,-number 5',
            'keyword_separator': ','
        }
        sql, bind = TextFilter.build_path_query(params, np)
        assert sql is not None
    ...
    def test_and_keyword_mode(self, engine):
        params = {
            'keys': ['Comment'], 'keywords': 'photo,number',
            'keyword_separator': ',', 'keyword_mode': 'AND'
        }
        sql, bind = TextFilter.build_path_query(params, np)
        assert " AND " in sql
...
class TestComposerSorting:
    ...
    def test_natural_name_sort_asc(self, engine, composer):
        ...
        assert names == sorted(names, key=lambda s: [
            int(c) if c.isdigit() else c.casefold()
            for c in __import__('re').compile(r'(\d+)').split(s)
        ])
...
class TestComposerCombineStatic:
    ...
    def test_single_entry(self):
        sql, params = SearchComposer._combine([
            ("SELECT path FROM files WHERE path LIKE ?", ['%test%'], None)
        ])
        assert "LIKE" in sql
        assert params == ['%test%']
...
class TestDirectoryFilterGlobalScope:
    ...
    def test_and_consistent_with_and_without_directory(self, engine, composer):
        ...
        paths_no_dir, _, _ = composer.execute(
            engine, entries_no_dir, NaturalNameSort, True
        )
        paths_all_dir, _, _ = composer.execute(
            engine, entries_with_all_dir, NaturalNameSort, True
        )
    ...
    def test_apply_global_static(self):
        ...
        result_sql, result_params = SearchComposer._apply_global(
            combined, [], [(gsql, [])]
        )
