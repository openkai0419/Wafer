...
def test_compile():
    py_compile.compile('wayfer/app/indexer/watch_folder.py')
...
def _make_watcher(fake_db):
    from wayfer.app.indexer.watch_folder import FolderWatcher
    progress = MagicMock()
...
class TestExtractStable:
    ...
    def test_all_stable(self, tmp_path):
        from wayfer.app.indexer.watch_folder import _extract_stable, _stat_signature, _STABLE_THRESHOLD
        f1 = tmp_path / 'a.txt'
        f2 = tmp_path / 'b.txt'
        f1.write_text('x')
        f2.write_text('y')
    ...
    def test_none_stable(self, tmp_path):
        from wayfer.app.indexer.watch_folder import _extract_stable, _stat_signature
        f1 = tmp_path / 'a.txt'
        f2 = tmp_path / 'b.txt'
        f1.write_text('x')
        f2.write_text('y')
    ...
    def test_empty_pending(self):
        from wayfer.app.indexer.watch_folder import _extract_stable
        pending = {}
        stable = _extract_stable(pending)
        assert stable == set()
        assert pending == {}
    ...
    def test_deleted_file_stabilizes(self):
        from wayfer.app.indexer.watch_folder import _extract_stable, _STABLE_THRESHOLD
        old_ts = time.monotonic() - _STABLE_THRESHOLD - 1
...
class TestEventAccumulator:
    ...
    def _make_accumulator(self):
        from wayfer.app.indexer.watch_folder import _EventAccumulator
