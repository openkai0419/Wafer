...
from wayfer.plugin.grid.handler import grid_resolver, VIEWER_THUMBNAIL_DEFAULT_SIZE


def test_compile():
    py_compile.compile('wayfer/app/viewer/commands/setting_commands.py')
...
class TestViewerThumbnailDefaultSize:
    ...
    def test_restore_from_setting(self):
        with patch('wayfer.app.viewer.commands.setting_commands.app_settings') as mock_setting:
            mock_setting.get.return_value = 4096
            from wayfer.app.viewer.commands.setting_commands import _restore_thumbnail_size
            _restore_thumbnail_size()
            assert grid_resolver.thumbnail_size == 4096

    def test_set_command_saves_value(self):
        with patch('wayfer.app.viewer.commands.setting_commands.app_settings') as mock_setting, \
             patch('wayfer.app.viewer.commands.setting_commands.QtWidgets') as mock_qt:
            mock_qt.QInputDialog.getItem.return_value = ('2048', True)
            ctx = MagicMock()
            ctx.get.return_value = None
            from wayfer.app.viewer.commands.setting_commands import set_viewer_thumbnail_default_size
    ...
    def test_set_command_clamps_minimum(self):
        with patch('wayfer.app.viewer.commands.setting_commands.app_settings'), \
             patch('wayfer.app.viewer.commands.setting_commands.QtWidgets') as mock_qt:
