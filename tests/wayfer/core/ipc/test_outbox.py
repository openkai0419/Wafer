...
from wayfer.core.ipc.transport import Priority
from wayfer.core.ipc.message import Message
from wayfer.core.ipc.node import Node
from wayfer.core.ipc.outbox import (
...
class TestOutboxStore:
    ...
    def _make_store(self, tmp_dir, node_id='test-node'):
        with patch('wayfer.core.ipc.outbox._OUTBOX_DIR', tmp_dir):
    ...
    def test_scan_all(self):
        with tempfile.TemporaryDirectory() as tmp:
            with patch('wayfer.core.ipc.outbox._OUTBOX_DIR', tmp):
    ...
    def test_cleanup_empty_files_removes_dead_pid(self):
        with tempfile.TemporaryDirectory() as tmp:
            with patch('wayfer.core.ipc.outbox._OUTBOX_DIR', tmp):
                ...
                with patch('wayfer.core.ipc.outbox.psutil.pid_exists', return_value=False):
    ...
    def test_cleanup_empty_files_keeps_alive_pid(self):
        with tempfile.TemporaryDirectory() as tmp:
            with patch('wayfer.core.ipc.outbox._OUTBOX_DIR', tmp):
                ...
                with patch('wayfer.core.ipc.outbox.psutil.pid_exists', return_value=True):
                    cleanup_empty_outbox_files()
                assert os.path.exists(db_path)
