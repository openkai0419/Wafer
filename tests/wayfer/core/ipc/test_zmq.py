...
from wayfer.core.ipc.transport import NODE_TIMEOUT, Priority
from wayfer.core.ipc.broker import Broker, _CoalescingQueue, _PeerInfo
from wayfer.core.ipc.message import Message
from wayfer.core.ipc.node import Node


def test_compile():
    py_compile.compile('wayfer/core/ipc/message.py')
    py_compile.compile('wayfer/core/ipc/broker.py')
    py_compile.compile('wayfer/core/ipc/node.py')
    py_compile.compile('wayfer/core/ipc/transport.py')
    py_compile.compile('wayfer/core/ipc/outbox.py')
...
class TestReconnection:
    ...
    def test_node_reconnects_after_broker_restart(self):
        ...
        with patch('wayfer.core.ipc.node.read_broker_port') as mock_read:
            broker2 = Broker()
            broker2.start()
