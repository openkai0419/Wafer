...
from wayfer.plugin.installer import (
    EmbeddedPython,
    _validate_url,
    _sha256_file,
    _download_file,
...
class TestEmbeddedPython:
    ...
    def test_ensure_ready_unsupported_platform(self, tmp_path):
        ep = EmbeddedPython(str(tmp_path / '_python'))
        with patch('wayfer.plugin.installer._platform_key', return_value=None):
    ...
    def test_download_and_extract(self, tmp_path):
        ...
        ep = EmbeddedPython(str(d))
        with patch('wayfer.plugin.installer._download_file') as mock_dl:
    ...
    def test_setup_pip_uncomments_site(self, tmp_path):
        ...
        ep = EmbeddedPython(str(d))
        with patch('wayfer.plugin.installer._download_file'):
            with patch('wayfer.plugin.installer._run_subprocess'):
                ep._setup_pip()
...
class TestInstallRequirements:
    ...
    def test_uses_embedded_python(self, tmp_path):
        ...
        with patch('wayfer.plugin.installer.EmbeddedPython', return_value=mock_ep):
