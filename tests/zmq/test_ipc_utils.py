...
from source.zmq.ipc_utils import read_broker_port, remove_broker_port, write_broker_port


def test_compile():
    py_compile.compile('source/zmq/ipc_utils.py')


def test_write_and_read(tmp_path):
    port_file = tmp_path / 'ipc' / 'broker.json'
    with patch('source.zmq.ipc_utils._PORT_FILE', port_file):
