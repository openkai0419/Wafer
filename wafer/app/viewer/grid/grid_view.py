    ...
    def paintEvent(self, event):
        ...
        if g._drop_preview_rect and not g._drop_preview_rect.isNull():
            ...
            if g._drop_preview_title or g._drop_preview_text:
                ...
                painter.restore()
        g._paint_grid_overlays(painter, self._map_rect)
        painter.end()
...
class GridView(QtWidgets.QGraphicsView, ActionKit.UIMixin):
    ...
    def __init__(self, root, items: GridItemModel | None = None, parent=None):
        ...
        self._overlay_host = getattr(root, "grid_overlay_host", None)
    ...
    def _clear_all_widgets(self):
        ...
        self._sync_overlay_visible_paths()
        self.rects = LayoutData.empty()
    ...
    def _effective_scroll_top(self, bar):
        if self._scroll_anim is not None and self._scroll_anim.state() == QtCore.QAbstractAnimation.Running:
            ...
            if i >= n:
                ...
                if self._needs_reload(self.widgets[i], layout[i].size()):
                    self._pipeline.schedule_render(
                        i,
                        self.items.paths[i],
                        self._content_size(layout[i].size()),
                    )
            else:
                self._recycle_widget(i)
                self.visible_indices.discard(i)
    ...
    def _sync_overlay_visible_paths(self):
        host = self._overlay_host or getattr(self.root, "grid_overlay_host", None)
        if host is None:
            return
        paths = tuple(self.items.paths[i] for i in sorted(self.visible_indices) if 0 <= i < len(self.items.paths) and self.items.paths[i])
        host.set_visible_paths(paths)

    def _paint_grid_overlays(self, painter, map_rect):
        host = self._overlay_host or getattr(self.root, "grid_overlay_host", None)
        if host is not None:
            host.paint(painter, self, map_rect)
