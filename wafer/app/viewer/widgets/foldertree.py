    ...
    def dropMimeData(self, data, action, row, column, parent):
        ...
        if data.hasFormat(self._mime_type):
            ...
            for i, src in enumerate(src_paths):
                ...
                plans.append(PastePlanItem(
                    index=i, src=Path(src), is_dir=True, action="cut",
                    dst_default=dst_default, conflict=conflict, suggested_dst=suggested,
                ))
...
class LazyFolderTreeView(QtWidgets.QTreeView):
    ...
    def __init__(self, roots=None, excluded=None):
        ...
        self.setStyleSheet(
            f'QTreeView::item:selected {{ background-color: {_p.accent}; color: {_p.accent_text}; }}'
        )
