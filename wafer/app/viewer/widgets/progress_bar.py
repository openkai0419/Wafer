...
class TooltipPopup(PopupWindow):
    def __init__(self, text, parent=None):
        ...
        layout.setContentsMargins(dpix(8), dpix(4), dpix(8), dpix(4))
        self.label = QtWidgets.QLabel(text)
        self.label.setStyleSheet(f"color: white; background: black; padding: {dpix(2)}px; border-radius: {dpix(3)}px;")
...
class ThinProgressBar(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self._value = 0
        self._maximum = 0
        self.setFixedHeight(dpix(3))
        self.setMinimumWidth(dpix(100))
    ...
    def _installHoverProxy(self):
        self._hover_proxy = HoverProxy(self, margin=dpix(3))
    ...
    def _drawGlow(self, painter):
        ...
        if bar_width <= 0:
            return
        glow_width = max(dpix(10), int(bar_width * 0.4))
    ...
    def mouseMoveEvent(self, event):
        ...
        offset = QtCore.QPoint(dpix(8), dpix(16))
