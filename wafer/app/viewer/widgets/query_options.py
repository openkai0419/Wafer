...
class SearchOptionPopup(QtWidgets.QDialog, TranslatorMixin):
    ...
    def get_settings(self):
        ...
        return {'query_mode': self.query_type_combo.currentData(), 'keyword_mode': 'AND' if self.and_radio.isChecked() else 'OR', 'sort_by': sort_by, 'ascending': ascending}
...
class SearchOptionsBar(QtWidgets.QWidget, TranslatorMixin):
    ...
    def get_values(self):
        ...
        kwargs.update({'keys': keys or None, 'keywords': self.search_bar.text(), 'keyword_separator': self.option_popup.get_keyword_delimiter()})
        return kwargs
