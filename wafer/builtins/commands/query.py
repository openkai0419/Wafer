    ...
    def commands(cls):
        return [
            ...
            ActionKit.Command(
                ...
                checked=lambda: _svc().get("auto_execute", True) if _svc() else True,
            ),
            ActionKit.Command(
                path="qry.toggle_auto_execute_on_update",
                display="Auto Execute on Database Update",
                func=toggle_auto_execute_on_update,
                checked=lambda: _svc().get("auto_execute_on_update", True) if _svc() else True,
            ),
            "-",
            ":Search Options",
            ActionKit.Command(
                ...
                checked=lambda: _svc().get("include_subfolders", True) if _svc() else True,
            ),
            ActionKit.Command(
                path="qry.toggle_include_contained_files",
                display="Include Contained/Virtual Files",
                func=toggle_include_contained_files,
                checked=lambda: _svc().get("include_contained_files", True) if _svc() else True,
            ),
            "-",
            ":Sort",
            "Sort By/:Sort By",
            *[
                ActionKit.Command(
                    ...
                    checked=_sort_resolver(k),
                )
                for k in _sort_choices()
            ],
            "Sort Order/:Sort Order",
            ActionKit.Command(path="Sort Order/qry.order_asc", display="Ascending", func=_make_order_func(True), action_group=GROUP_ORDER, checked=_order_resolver(True)),
            ActionKit.Command(
                path="Sort Order/qry.order_desc",
                display="Descending",
                func=_make_order_func(False),
                action_group=GROUP_ORDER,
                checked=_order_resolver(False),
            ),
            ActionKit.Command(
                path="qry.cycle_sort",
