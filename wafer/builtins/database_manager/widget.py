...
class _DatabaseDetailWidget(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(src_group, 1)
        layout.addWidget(ign_group, 1)

    def _save_current_to_buffer(self):
        if self._db_name:
            sources = [self._source_list.item(i).text()
                       for i in range(self._source_list.count())]
            ignores = [self._ignore_list.item(i).text()
                       for i in range(self._ignore_list.count())]
    ...
    def _add_source(self):
        ...
        folder = QtWidgets.QFileDialog.getExistingDirectory(
            self, f'Add Source Folder to "{self._db_name}"',
        )
        if not folder:
            return
        existing = [self._source_list.item(i).text()
                    for i in range(self._source_list.count())]
    ...
    def _add_ignore(self):
        if not self._db_name:
            return
        folder = QtWidgets.QFileDialog.getExistingDirectory(
            self, f'Add Ignore Folder to "{self._db_name}"',
        )
        if not folder:
            return
        existing = [self._ignore_list.item(i).text()
                    for i in range(self._ignore_list.count())]
        if folder not in existing:
