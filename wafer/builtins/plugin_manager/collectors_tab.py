...
class CollectorsTab(QtWidgets.QWidget):
    ...
    def __init__(self, collector_names: list[str] | None = None, parent=None):
    ...
    def _build_matrix(self):
        ...
        if not self._collector_names:
            self._main_layout.addWidget(QtWidgets.QLabel('No collector plugins loaded.'))
        ...
        matrix_group = QtWidgets.QGroupBox('Per-Database Assignment')
        ...
        self._main_layout.addWidget(matrix_group)
        self._main_layout.addStretch()
    ...
    def _on_all_toggled(self, collector: str, checked: bool):
        for db in self._db_names:
            cb = self._matrix.get((collector, db))
            if cb:
                cb.setChecked(checked)
        self._sync_all_check(collector)

    def _sync_all_check(self, collector: str):
        all_cb = self._all_checks.get(collector)
        if not all_cb:
            return
        states = []
        for db in self._db_names:
    ...
    def get_per_db_collectors(self) -> dict[str, list[str]]:
        ...
        for db in self._db_names:
            enabled = []
            for coll in self._collector_names:
                cb = self._matrix.get((coll, db))
    ...
    def get_newly_disabled(self) -> list[tuple[str, str]]:
        ...
        for db in self._db_names:
            ...
            for coll in prev - curr:
                disabled.append((db, coll))
        return disabled
    ...
    def confirm_and_save(self) -> bool:
        ...
        layout.addWidget(QtWidgets.QLabel('The following collectors will be disabled:'))

        purge_checks: list[tuple[QtWidgets.QCheckBox, str, str]] = []
        for db, coll in disabled:
            cb = QtWidgets.QCheckBox(f'Delete data: {coll} on {db}')
            purge_checks.append((cb, db, coll))
            layout.addWidget(cb)

        layout.addWidget(QtWidgets.QLabel(
            'Unchecked items keep their data.\n'
        ...
        to_purge = [(db, coll) for cb, db, coll in purge_checks if cb.isChecked()]
        self.save_to_dbs()
        if to_purge:
