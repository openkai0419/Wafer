...
class _SequentialConfigWidget(RenameConfigWidget):
    ...
    def __init__(self, source, parent=None):
        ...
        for lbl, attr in [("Start", "start"), ("Step", "step"), ("Pad", "padding")]:
            ...
            sp.setPrefix(f"{lbl}: ")
            sp.setStyleSheet(si)
            sp.valueChanged.connect(lambda v, a=attr: (setattr(source, a, v), self.changed.emit()))
            lay.addWidget(sp)
...
class MetaSource(BaseRenameSourcePlugin):
    ...
    def create_config_widget(self, parent=None, **context):
        ...
        kb.value_changed.connect(lambda v: (setattr(self, "key", v), w.changed.emit()))
        lay.addWidget(kb)
        return w
