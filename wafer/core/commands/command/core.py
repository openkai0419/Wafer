...
class CommandParam:
    __slots__ = ("choices", "choices_fn", "default", "description", "max_value", "min_value", "name", "required", "type", "widget_type")
    ...
    def __init__(
        ...
        *,
        ...
        if callable(value) and not isinstance(value, type):
            self.choices_fn = value
            self.choices = None
            self.default = default if default is not inspect._empty else ""
            self.type = type(self.default) if self.default is not None else str
        elif isinstance(value, (list, tuple)):
            xs = list(value)
            if not xs:
                raise ValueError("value choices must not be empty")
            self.choices_fn = None
            self.choices = xs
    ...
    def resolve_choices(self) -> list | None:
    ...
    def get_commands_by_category(self, category: str, widget_scope: str | None = None) -> dict[str, type[CommandBase]]:
        ...
        for cid, cmd_class in self._commands.items():
            ...
            if meta and meta.category == category:
                if not meta.target_widgets or (widget_scope and widget_scope in meta.target_widgets):
...
def call_with_matching_args(fn: Callable[..., Any], values: Mapping[str, Any] | None = None) -> Any:
    if values is None:
        values = {}
    return _call_with_matching_args(fn, dict(values))
...
def create_command_from_meta(meta: CommandMeta) -> type[CommandBase]:
    ...
    if callable(fn):
        ...
        def _wrapped_execute(self, **kwargs):
            ...
            return call_with_matching_args(fn, filtered_kwargs)
        ...
        def _dispatch_execute(self, **kwargs):
            ...
            if not callable(cb):
                return None
            return call_with_matching_args(cb, kwargs)
