    ...
    try:
        ...
        if same_dir:
            ...
            hr = shell32.SHBindToParent(
                full_pidls[0], byref(_IID_IShellFolder), byref(folder_ptr), byref(child_pidl_ptr)
            )
            if hr != S_OK:
        ...
        reserved = c_uint(0)
        hr = _vt(
            folder_obj, _ISF_GetUIObjectOf, HRESULT,
            wt.HWND, c_uint, POINTER(c_void_p), c_void_p, POINTER(c_uint), POINTER(c_void_p),
        ...
        hr = _vt(
            cm_obj, _ICM_QueryContextMenu, HRESULT,
            wt.HMENU, c_uint, c_uint, c_uint, c_uint,
            ...
            def subclass_proc(h, msg, wp, lp):
                if msg in (WM_INITMENUPOPUP, WM_DRAWITEM, WM_MEASUREITEM, WM_MENUCHAR):
                    try:
                        if cm3_obj:
                            result = wt.LPARAM()
                            _vt(cm3_obj, _ICM3_HandleMenuMsg2, HRESULT,
                                c_uint, wt.WPARAM, wt.LPARAM, POINTER(wt.LPARAM)
                                )(cm3_obj, msg, wp, lp, byref(result))
                            if msg == WM_MENUCHAR:
                                return result.value
                            return 0
                        elif cm2_obj:
                            _vt(cm2_obj, _ICM2_HandleMenuMsg, HRESULT,
                                c_uint, wt.WPARAM, wt.LPARAM
                                )(cm2_obj, msg, wp, lp)
                            return 0
                    except OSError:
                        pass
        ...
        if cmd:
            ...
            hr = _vt(
                cm_obj, _ICM_InvokeCommand, HRESULT,
