...
class LayoutManager(QtCore.QObject):
    ...
    def toggle_panel(self, name: str):
        ...
        if name in self._tree.floating:
            if not entry.closable:
                return
            entry.last_floating = capture_floating_state(entry.floating_window or entry.dock_widget)
            self._cleanup_entry(entry)
            self._tree.floating.pop(name, None)
            return
    ...
    def _create_managed_dock(self, entry: PanelEntry) -> PanelDockWidget:
        ...
        dock.topLevelChanged.connect(lambda floating, n=entry.name: self._on_dock_float_changed(n, floating))
    ...
    def _subtree_extent(
        ...
        if node.orientation == orientation:
            return sum(self._subtree_extent(c, docks, orientation) for c in node.children)
    ...
    def _apply_pending_for(self, name: str, entry: PanelEntry):
        ...
        if name in set(original_tree.docked_names()):
            ...
            self._tree.root = reinsert_from_blueprint(self._tree.root, saved_root, name)
